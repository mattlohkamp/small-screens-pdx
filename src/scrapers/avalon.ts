...
const VENUE_ID = "avalon";
const BASE = "https://wcms-p-101180-114050d4.netlify.app";
const THEATER_ID = "X0430";
...
export async function scrapeAvalon(): Promise<Film[]> {
  const start = today();
...
// Run directly: tsx src/scrapers/avalon.ts
if (process.argv[1].includes("avalon")) {
  const films = await scrapeAvalon();
