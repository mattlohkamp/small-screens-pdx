...
// Single curl attempt; throws on failure (curl --fail exits 22 on CF's 403/503
// challenges). The scrape orchestrator retries the whole scraper with backoff, so
// there's no inner retry here — nesting the two multiplied attempts (up to 12×)
// and turned a persistent block into a multi-minute hang.
